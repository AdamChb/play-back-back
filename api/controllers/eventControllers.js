...
const updateEvent = async (req, res) => {
  ...
  try {
    ...
    const affectedRows = await Event.updateEvent(
      id,
      nom_session,
      description,
      date_heure,
...
const checkAttendance = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ enrolled: true, message: "Attendance checked successfully" });
  } catch (error) {
...
const enrollEvent = async (req, res) => {
  ...
  try {
    ...
    if (attendance.enrolled) {
      res
        .status(200)
        .json({ message: "User enrolled in event", enrolled: true });
    } else {
      res
        .status(200)
        .json({ message: "User unenrolled from event", enrolled: false });
    }
  } catch (error) {
