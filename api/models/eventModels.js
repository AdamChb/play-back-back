...
const getEventGames = async (eventId) => {
  const query = `
        SELECT j.*
        FROM evenement_jeux ej
        JOIN jeu j ON ej.ID_jeu = j.ID_jeu
...
const getNextEvents = async () => {
  const query =
    "SELECT * FROM vue_evenements_futurs ORDER BY date_heure ASC LIMIT 10";
  try {
